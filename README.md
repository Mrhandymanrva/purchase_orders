...
Dropdown discovery can run with `QBO_PARENT_CC_ACCOUNT_ID` before you know the child account IDs. Select/export those discovered child IDs to configure `QBO_CARD_ACCOUNT_IDS` for purchase imports. Full sync still requires the explicit child allowlist, `ST_BUSINESS_UNIT_IDS`, and a valid `SYNC_FROM`; refreshing dropdowns does not widen transaction scope.
