...
Exact group sums support 1:1, 1:many and many:1; many:many is out of scope. Maximum group size defaults to 3, configurable up to 4. In Match and flag mode, all 1:1 candidates are considered before amount-pruned group search. Each anchor has a 20,000-step / 200-group budget; a 20,000-candidate overall group budget preserves selected balanced matches with an explicit search-limit flag. Unmatched affected records also expose the limit. The strict mode retains its original 14-candidate pool limit. A snapshot is capped at 2,000 records; excessive 1:1 candidates fail closed. Among eligible balanced matches, selection prefers shared references, verified card-user/PO-technician agreement, fewer source records, confidence, closest dates and stable IDs. Preferences do not inflate confidence scores. This deterministic allocation is not a globally optimal solver; competing allocations within the configured score margin are flagged in Match and flag mode and require review in strict mode. A source record appears in at most one result, and manual reservations take precedence.
