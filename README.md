...
3. In **Card assignments**, edit all mappings in one grid using QuickBooks subaccount and ServiceTitan person dropdowns and effective dates. **Save all changes** saves every edited row together; **Save row** saves one assignment. No reason is required for assignment-grid saves. Source IDs distinguish duplicate account and employee names. Per-charge manual corrections take precedence, then dated subaccount assignments, then imported ownership, then Unassigned. Reconciled POs follow their actual card users.
...
The grid shows every saved assignment period plus an editable row for each unmapped active card subaccount. **Add assignment period** creates another dated row. A sticky toolbar shows the unsaved count and **Save all changes** outside the horizontal table scroll. **Save row** remains visible in the pinned right column and preserves the other drafts. Switching app sections retains drafts; reloading or closing with unsaved changes triggers the browser's leave-page warning.

Save all sends only edited rows in one revision-checked PostgreSQL transaction. Every row and the final assignment periods are validated before any changes are applied. Ending an old period and starting its replacement can be saved together in either row order. Invalid people, accounts, overlapping dates, duplicate edits or a stale revision reject the batch without partial saves. The grid retains edits after a failed save. No reason field is shown or required; the audit records actor, timestamp, before/after assignments and affected records, while existing historical notes are preserved. No new PostgreSQL migration is required.
