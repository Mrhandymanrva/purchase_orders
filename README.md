...
1. Use the **Individual filter** to select a card user or ST technician. Card purchases follow the actual card user; unlinked POs follow the source ST technician. Verified ST identities connect a card label and the technician name when their display names differ. KPI totals follow the person and date range; status and search filters narrow the table. Variance is the full reconciliation-group difference, not a personal liability allocation.
...
5. **Export report** downloads a CSV using current person, date, status, and search filters. It contains one line per card charge, its actual user, ownership source, signed amount, PO IDs, proposed/reconciled link state, status, and score. It does not repeat the full PO amount for each user; shared-PO totals therefore are not double-counted. Orphan POs appear with zero card spend only in the all-individuals report. User-provided strings are escaped against spreadsheet formula injection.
   Click any reconciliation column header to sort by vendor/transaction, card user, PO technician, date, amount, status, confidence or PO. Click again to reverse direction; the active arrow shows the order. New numeric/date sorts start highest/newest first; text sorts start A–Z. Blank confidence and PO values remain last. Sorting applies after filtering and does not alter matches or totals. CSV and PDF exports keep the same reconciliation-group order (CSV still expands each group into individual charge rows). Group amounts use the selected person/date filters, including signed refunds.

6. Open a result and click **Confirm reconciliation**. After a successful confirmation the detail panel closes and returns you to the filtered, sorted table. Failed saves keep it open and show an inline error. No written reason is required; the audit records the operator, timestamp, selected purchases and POs, and a standard confirmation note. Dismissing an exception still requires a reason. To choose a PO, use the searchable purchase-order picker, then select one or more charges. The panel shows signed charge totals, the PO total, and the remaining balance before confirmation. Open a confirmed match to add later charges or revise its selection. A purchase can link to at most one PO; several explicitly selected charges can link to that PO. Automatic matching remains one-to-one. Live IDs use `ST:<id>`; sample IDs use `PO-2041` etc. Another manual decision cannot reserve those records again. Manual decisions affect this app only.
7. In a reconciliation detail panel, open **Create vendor exclusion rule**, choose an optional purchase cap, then click **Save and apply rule**. This explicitly approves the rule, refreshes reconciliation immediately and returns to the table. The vendor is prefilled from the source record. **Rules & scoring** also offers **Save and apply rule** for vendor aliases and No-PO-Required rules. Automatic suggestions remain inert until **Approve rule** is explicitly clicked. Every approval recalculates the imported snapshot without calling ST or QuickBooks; the audit records before/after rules and complete reconciliation evidence.
...
- Adapters validate response shapes, paginate, apply timeouts, retry 429/5xx up to three times, and reject redirects. They do not log token values or response bodies. Missing ServiceTitan vendor-directory references, non-USD amounts, fractional cents, other malformed records, and pagination truncation fail the sync.
- QuickBooks payee names take precedence over descriptions. If the payee is blank, complete Lowe's store descriptors such as `LOWES #X1037*` (including repeated Lowe's store descriptors) identify **Lowe's** through the deterministic `qbo-lowes-store-v1` parser. It rejects prose, mixed merchants, unknown suffixes and partial matches. The original description and parser version are retained in source evidence, fingerprints, the sync audit and CSV exports. Existing vendor-alias approval, amount/date, duplicate and refund checks still apply; recognizing the merchant never confirms a PO link by itself. Other records without a supplied payee name show their description with **Vendor needs confirmation**, or **Missing vendor** when no description exists. They remain outside automatic PO matching, vendor-based No-PO exemptions and rule suggestions. An operator may explicitly approve a No-PO rule for the full QuickBooks description; this is an exception policy, not a vendor identity or PO match. Signed amounts and card ownership are preserved. Refunds without a compatible credit PO are labeled **Unallocated refund**.
- ServiceTitan POs are read back to 90 days before `SYNC_FROM` to catch older related POs. Unlinked POs dated outside the imported card interval are labeled **Outside card coverage** and excluded from **Needs review** and unresolved variance. They remain in All items, exports and PO scorecard totals, and can still match charges inside the imported interval. The **Imported history** panel shows both source counts and date ranges, with a shortcut to these POs. Unknown coverage never suppresses an exception. No records are fetched after the current date. Keep `SYNC_FROM` fixed for stable history; narrowing it removes records from the active snapshot and invalidates dependent manual decisions, with prior evidence retained in the audit.
...
Default score: vendor 30 + amount 45 + date proximity up to 15 + explicit PO reference 10. Weights must sum to 100. Automatic threshold is 85, date window 21 days, grace period 5 days, late threshold 2 days, and amount tolerance 1 cent. Choose **Rules & scoring → Automation policy → Match and flag uncertainty**, then **Save policy** to apply it to the current imported snapshot without a source sync. Eligible matches at or above the threshold become **Matched** or **Matched with flags**. Close alternatives, late POs and different/unverified technician identities are informational flags; they do not require individual confirmation. Missing vendors, missing POs, amount mismatches, partial matches, possible duplicate charges and explicit conflicting references remain exceptions. An ST PO number earns reference points only when it agrees with the charge reference; merely existing in ST is not cross-system evidence.

**Matched with flags** counts as reconciled in KPI totals, the Matched filter, CSV/PDF exports and technician PO attribution. Its own tab/filter isolates these items for optional inspection. The register and detail panel show flags, scoring evidence and competing source IDs. Exported CSV includes flags, evidence and engine version; PDF includes uncertainty alongside the reconciled status. **Require review for uncertainty** preserves the original strict policy and remains the default for workspaces that have not explicitly selected the new mode. Policy saves audit the before/after configuration, status counts, source fingerprint, rule IDs, engine version, as-of date and complete results. Existing manual decisions retain priority while their source evidence is unchanged.

Richmond uses **one card transaction ↔ one PO (1:1)**. Purchases and POs are never combined, including refunds and partial payments. All single-PO candidates within the vendor/date/sign constraints are considered in both automation modes; legacy group-search limits do not apply. A purchase without an eligible single PO remains unmatched. The policy applies to existing imports immediately when saved and to later imports automatically. The policy API also enforces `maxGroup: 1`; group-size controls are no longer offered. Legacy grouped configurations remain readable for audit/history compatibility, but new policy saves cannot enable them. Old grouped manual decisions remain in history and are recorded as excluded in the policy audit; they no longer reserve records under 1:1. Valid single-PO and source-only manual decisions keep their priority.

A snapshot is capped at 2,000 records; excessive candidate counts fail closed. Among eligible balanced matches, selection prefers shared references, verified card-user/PO-technician agreement, confidence, closest dates and stable IDs. Preferences do not inflate confidence scores. This deterministic allocation is not a globally optimal solver; competing allocations within the configured score margin are flagged in Match and flag mode and require review in strict mode. Each source record appears in exactly one result, including unmatched records. Historical exports can still display groups from older policies.

No-PO-Required rules require explicit approval and a positive charge within the configured cap. Vendor rules use exact normalized vendor identity and approved aliases. For a purchase whose payee is unassigned, the modal instead prefills the full QuickBooks description. Such description rules apply only to unassigned-payee purchases with that complete description, ignoring case and repeated whitespace but preserving punctuation and store numbers; substrings and different locations do not match. An explicit **No amount limit** option uses `maxCents: null` for merchant-wide exemptions; missing limits are rejected and zero remains a zero-dollar cap. Fuel-merchant exemptions cover all purchases at approved merchants, including convenience-store items, and continue applying to future imports under the same vendor names. Exempt purchases stay in reports and technician spend totals and do not require a PO. Suspected duplicates, refunds and zero-value records remain review items. Missing-vendor records require a separately approved full-description exception; a vendor rule alone cannot exempt them. Duplicate detection for unassigned payees uses the complete description, account, date and signed amount. ServiceTitan PO source records are retained. Updating one pending suggestion for the same vendor preserves its ID and audits the before/after policy, without approving it; approved policies cannot be overwritten through a suggestion. **Save and apply rule** is a separate explicit approval action and can update an existing rule for the same scope. Both it and **Approve rule** refresh reconciliation immediately, while preserving valid manual decisions. Manual decisions contain complete source fingerprints and are automatically ignored after source changes. Rerun audit events retain the source fingerprint, policy, approved rule IDs, as-of date, engine version, invalidations and complete results.

The reconciliation table shows a separate **PO technician** column next to **Card user** for linked and unlinked POs. Names resolve from the PO's `technicianId` and the technician directory, with a saved card-mapping name as a historical fallback only for the same typed ST identity. A matched card user never substitutes for the PO's technician. Unknown IDs display `ST technician #<id>`; a PO without an ID displays `Technician not specified`. These labels, name search, sorting, detail panels and exports are read-only presentation and do not change matching decisions. Active-only ST person imports remain unchanged. CSV adds PO technician and ST technician ID columns; PDFs label the technician beside each PO.
...
- Verify a known 1:1 charge, credit, duplicate, late/backdated PO, missing PO and partial payment. Verify that split payments and several POs with a coincidental aggregate total are not combined. Confirm reference conflicts never auto-match. Review proposed matches before approving any policy changes.
...
- Confirm manual decisions survive an unchanged resync, invalidate on material source/ownership changes, and can be replaced only with another audited review. Confirmations do not require a written reason; dismissals do. Confirm proposed rules have no effect until approved.
...
### Categorizing an individual purchase without a PO

In reconciliation detail, choose **Office**, **Non-Billable Materials**, or **Tools** under **Spend category**, then click **Save category**. This is an explicit No-PO-Required decision for that individual card purchase; no written reason is required. Saving refreshes the register/KPIs and closes the detail panel. Other purchases at that vendor are unaffected, and any linked PO returns to ordinary matching. Signed amounts, source records, spend totals and source-system accounting categories are unchanged.

Use **Rules & scoring → Spend categories** to add or rename choices, toggle **Available**, then **Save categories**. Stable IDs preserve existing assignments when names change. Unavailable categories disappear from new selections but remain labeled on existing purchases. The sortable **Spend category** register column, category-name search, CSV column and PDF status/category field show current category names. The approval audit retains the original name and complete decision evidence.

Select **Uncategorized** and **Save category** to clear an assignment and return that purchase to normal matching. Category approvals survive unchanged imports; a change to source financial or ownership evidence invalidates the approval until reviewed and saved again. The original decision remains in the audit. A category save is a manual exception for one purchase, including an explicitly reviewed credit if selected; it creates no automatic vendor policy. No QuickBooks or ServiceTitan write is performed.

## Imported payee names and descriptions

**Vendor needs confirmation** means the QuickBooks import did not supply a usable separate payee name. A merchant may still be visible in the transaction description; the detail panel labels these two fields separately. This condition does not prove that no payee is assigned in QuickBooks. Unverified description text does not become an authoritative vendor identity or PO match. Manual PO review, individual spend categories and explicitly approved description exclusion rules remain available. Existing internal status keys remain compatible with saved records and filters; the clearer label is used throughout the UI, CSV and PDF reports.

### Unmatched totals and manual charge selection

The reconciliation table shows POs without charges, charges without POs, and net difference (POs minus charges). These totals follow all current table filters, preserve negative credits, and exclude dismissed and No-PO-Required items. Proposed groups with records on both sides are not counted as orphan records. Outside-card-coverage POs are included only when the selected table filters include them.

Manual matching searches available records across all dates and individuals. Each selection is audited and reserved as a single reconciliation group; other manual decisions cannot reuse its records. Mismatched totals remain visible for explicit review. New multi-charge confirmations survive reloads and unchanged syncs, and invalidate when source evidence changes. Previously inactive legacy grouped decisions remain inactive.

### Customer and job details

Reconciliation includes sortable, searchable Customer and Job ID columns, PO context in the detail and matching panels, and customer/job fields in CSV and PDF exports. These are ServiceTitan details from the linked PO (including proposed links), not the credit-card payee. A PO jobId links to the job customerId, then to the customer name. POs without a job and charges without a linked PO show a dash. Existing snapshots acquire these fields on the next successful import. Jobs and Customers read access is required; lookup failures leave the previous snapshot intact. Missing/deleted related records remain blank. Display metadata does not change match evidence or invalidate existing confirmations.

The Purchase order column and matching selector display ServiceTitan PO numbers (for example 144860-003), with internal record IDs retained separately for stable links. Job number / ID shows the visible job number and the internal job ID separately. Search accepts all of these identifiers; CSV includes both PO numbers and record IDs.

In production, job numbers and job IDs open their ServiceTitan job records in a new tab. PO numbers open the related ServiceTitan invoice used to view the PO, using its imported invoiceId (not the PO or line-item ID). Missing invoice/job IDs and sample/integration records remain plain text. Link tooltips identify the destination.
